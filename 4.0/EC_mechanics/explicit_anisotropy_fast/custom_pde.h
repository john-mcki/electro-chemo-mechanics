...
public:
  ...
  CustomPDE(const UserInputParameters<dim> &_user_inputs,
      ...
      : PDEOperatorBase<dim, degree, number>(_user_inputs, _pf_tools),
        ...
        C_rate(get_user_inputs().user_constants.get_double("C_rate")),
        delta_X_Li(get_user_inputs().user_constants.get_double("delta_X_Li")),
        vegard(get_user_inputs().user_constants.get_double("vegard")),
        site_volume(get_user_inputs().user_constants.get_double("site_volume")),
...
//  Chebyshev21Ocv<ScalarValue> ocv_model_;
  ...
  void post_solve_block(
    ...
    if (solver_id == 0) {
      ...
      if (increment == 0) {
        ...
        number integrated =
          psi * solve_context.get_invm_manager().get_jxw(TensorRank::Scalar);
        number volume = integrated * 1e-18; // volume in m^3 (from um^3)
        number capacity = volume / site_volume * delta_X_Li * F / 3600.0;
        i_target = -capacity * C_rate;
        LogStream(0) << "Domain volume is:" << volume << " m^3 \n";
        LogStream(0) << "Capacity is:" << capacity << " A h \n";
        LogStream(0) << "C-rate is:" << C_rate << " 1/h \n";
        LogStream(0) << "target current is:" << i_target << " A \n";
      }
    }
    ...
    if (solver_id == 2) {
      ...
      number total_current = integrated / (site_volume * 1e18 ) * F;
      number increment = solve_context.get_simulation_timer().get_increment();
      ...
      number V_inc = V_step * dt * i_proportion; // voltage increment based on current density
  ...
  void set_initial_condition(
    ...
    if (index == Fields::c.index)  scalar_value = c0;
    ...
    if ((index == Fields::Cel2.index) && (component == 0)) vector_component_value = 20.0;
    if ((index == Fields::Cel2.index) && (component == 1)) vector_component_value = 30.0;
    if (index == Fields::Cel3.index) vector_component_value = 0.0;
    if (index == Fields::eig1.index) vector_component_value = 0.01;
    if (index == Fields::eig2.index) scalar_value = 0.0;
  }
      ...
      [[maybe_unused]] unsigned int solve_block_id) const override
  {
    ...
    if (solve_block_id == 0) // c
      {
        // Calling variables
        ScalarValue c_val = variable_list.template get_value<Scalar, OldOne>(Fields::c.index);
        ScalarValue mu_val = variable_list.template get_value<Scalar, OldOne>(Fields::mu.index);
        ScalarGrad mu_grad = variable_list.template get_gradient<Scalar, OldOne>(Fields::mu.index);
        ScalarValue psi = variable_list.template get_value<Scalar, Current>(Fields::psi.index);
        ScalarGrad psi_grad = variable_list.template get_gradient<Scalar, Current>(Fields::psi.index);
        ...
        VectorGrad D = TensorHelper::extract_rank2_symm(variable_list, Subsets::diffusion());
    ...
    if (solve_block_id == 1) // u
      {
        ScalarValue c_val = variable_list.template get_value<Scalar, OldOne>(Fields::c.index);
        ScalarValue psi   = variable_list.template get_value<Scalar, Current>(Fields::psi.index);
        VectorGrad eigenstrain0 = TensorHelper::extract_rank2_symm(variable_list, Subsets::eigenstrain());
    ...
    if (solve_block_id == 2) // mu
      {
        ScalarValue c_val = variable_list.template get_value<Scalar, Current>(Fields::c.index);
        VectorGrad  ux    = variable_list.template get_symmetric_gradient<Vector, Current>(Fields::u.index);
        ScalarValue psi   = variable_list.template get_value<Scalar, Current>(Fields::psi.index);
        VectorGrad eigenstrain0 = TensorHelper::extract_rank2_symm(variable_list, Subsets::eigenstrain());
        VectorGrad eigenstrain = (c_val - c_ref) * eigenstrain0;
        VectorGrad stress;
        dealii::Tensor<2, Mechanics::voigt_tensor_size<dim>, ScalarValue> stiffness =
            TensorHelper::extract_stiffness(variable_list, Subsets::stiffness());
        Mechanics::compute_stress<dim, ScalarValue>(stiffness, psi * (ux - eigenstrain), stress);
        ScalarValue elastic_diffpot = dealii::scalar_product<2, dim, ScalarValue>(stress, eigenstrain0);
        elastic_diffpot *= site_volume * stress_scale;
    ...
    if (solve_block_id == 3) // post-processing
      {
        ...
        VectorGrad ux = variable_list.template get_symmetric_gradient<Vector, Current>(Fields::u.index);
        ScalarValue psi = variable_list.template get_value<Scalar, Current>(Fields::psi.index);
        ScalarValue mu_val = variable_list.template get_value<Scalar, Current>(Fields::mu.index);

        // pp fields directly from solution fields
        variable_list.set_value_term(Fields::particle_concentration.index, psi*c_val);
        variable_list.set_value_term(Fields::overpotential.index, del_phi + RT/F*mu_val);
  ...
  void compute_lhs(
    ...
    if (solve_block_id == 1) // mechanics - lhs
      {
        VectorGrad ux = variable_list.template get_symmetric_gradient<Vector, LHS>(Fields::u.index);
        ScalarValue psi = variable_list.template get_value<Scalar, Current>(Fields::psi.index);
        VectorGrad stress;
        dealii::Tensor<2, Mechanics::voigt_tensor_size<dim>, ScalarValue> stiffness =
            TensorHelper::extract_stiffness(variable_list, Subsets::stiffness());
  ...
  number vegard;
  number site_volume;  // m^3 / mol Li
  number stress_scale;
  number V_ref;
  number i_target = 0.0; // will be modified in post_solve_block to be the target current density for the simulation
  number delta_X_Li;
  number C_rate;
  number V_step;
  number V_min;
