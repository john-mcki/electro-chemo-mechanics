...
private:
                   ...
                   const ScalarValue &eig2) const
  {
    ...
    if (dim == 2)
      {
        energy =
          eig1[0] * stress[0][0] + 2.0 * eig2[0] * stress[0][1] + eig1[1] * stress[1][1];
      }
    else
      {
  ...
  VectorGrad
  eigenstrain_helper(const VectorValue &eig1, const ScalarValue &eig2) const
  {
  ...
  get_voigt2D(const VectorValue &Cel1,
              const VectorValue &Cel2,
  ...
  VectorValue
  get_flux2D(const VectorValue &cx, const VectorValue &D1, const ScalarValue &D2) const
  {
    VectorValue flux;
    if (dim == 2)
      {
