    ...
    if (solve_block_id == 0) // concentration and mechanics
      {
        ...
        ScalarValue flux_term   = -i_0 * (c_val - c_ref);
        ScalarValue c_func_val  = psi_grad_mag * flux_term;
        ScalarGrad  c_func_grad = psi * mobility * RT * mu_grad;
    ...
    if (solve_block_id == 1)
      {
        ...
        // Flux term
        ScalarValue flux = -i_0 * (c_val - c_ref);
        variable_list.set_value_term(6, psi_grad_mag * flux);
      }
  }
    ...
    if (solve_block_id == 0)
      {
        ...
        ScalarValue flux_term = -i_0 * del_c;

        // Additional functions
        ScalarValue mobility = (diffusivity * (c_val + epsilon)) / RT;
        ScalarValue mu_exp_term =
          exp((vegard * site_vol) / RT * hydrostatic_stress + mu_val);
        ScalarValue thermo_factor =
          (vegard * site_vol) / RT; // coefficient to hydorstatic stress

        // LHS comprised of 3x3 Jacobi

        ScalarValue j_c_c_val  = -psi * del_c + dt * i_0 * psi_grad_mag * del_c;
        ...
        variable_list.set_value_term(2, -j_c_c_val);
