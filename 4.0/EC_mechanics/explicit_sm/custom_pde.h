...
public:
  ...
  CustomPDE(const UserInputParameters<dim> &_user_inputs, PhaseFieldTools<dim> &_pf_tools)
    ...
    , Vmax(get_user_inputs().user_constants.get_double("Vmax"))
    , Vmin(get_user_inputs().user_constants.get_double("Vmin"))
  {}
...
private:
                   ...
                   [[maybe_unused]] unsigned int                       solver_id) override
  {
    if (solver_id == 2)
      {
        const auto &psi = solve_context.get_solution_indexer().get_solution_vector(4);
        const auto &c   = solve_context.get_solution_indexer().get_solution_vector(12);
        ...
        number total_current =
          integrated /
          (site_vol * F); // conversion of site fraction to total current density
        // std::cout << "Integrated concentration: " << total_current << std::endl;
    ...
    // double i_proportion = (i_target - total_concentration) / i_target;
    // double i_increment = V_inc *
    // solve_context.get_simulation_timer().get_timestep(); double V_app       = V_app
    // + ;

    // vectortype prod = c;
    // prod.scale(psi);
    // integrated = prod * mass_matrix;
  }
    ...
    if (solve_block_id == 0) // c
      {
        ...
        ScalarValue eq_c  = c_val + dt * (func_c + psi_grad_mag * react);
    ...
    if (solve_block_id == 2) // mu
      {
        ...
        Mechanics::compute_stress<dim, ScalarValue>(stiffness,
                                                    psi * transformation_strain,
                                                    stress);
        ScalarValue hydrostatic_stress = stress_scale * dealii::trace(stress) / 3.0;

        ScalarValue epsilon = 1.0e-8;
        ScalarValue mu_val =
          log(std::max(c_val, epsilon)) - (site_vol * vegard * hydrostatic_stress) / RT;
    ...
    else if (solve_block_id == 3) // pp
      {
        ...
        ScalarGrad  mu_grad  = variable_list.template get_gradient<Scalar, Current>(2);
        ScalarValue psi      = variable_list.template get_value<Scalar, Current>(3);
        ScalarGrad  psi_grad = variable_list.template get_gradient<Scalar, Current>(3);
        ScalarValue psi_grad_mag = psi_grad.norm() + offset;
        ...
        variable_list.set_value_term(11, hydrostatic_stress / stress_scale); // reported
                                                                             // in GPa
        variable_list.set_value_term(12, c - c_old);
        variable_list.set_value_term(13, del_phi);
      }
  }
