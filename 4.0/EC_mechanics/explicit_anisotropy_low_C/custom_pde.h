...
private:
  ...
  void post_solve_block(
    ...
    if (solver_id == 0) {
      ...
      if (increment == 0) {
        const auto &psi =
            solve_context.get_solution_indexer().get_solution_vector(
                Fields::psi.index);
        number integrated =
            psi * solve_context.get_invm_manager().get_jxw(TensorRank::Scalar);
        // volume calculation in m^3, assumes 1e-6 m thickness for 2D
        number volume = integrated * 1e-18;
    ...
    if (solver_id == 2) {
      ...
      // conversion assumes 1e-6 m thickness for 2D
  ...
  void set_initial_condition(
    ...
    if (index == Fields::psi.index)
      scalar_value = domain_parameter;
    if (index == Fields::D1.index)
      vector_component_value = 1e-4;
    if (index == Fields::D2.index)
      scalar_value = 0.0;
    if (index == Fields::Cel1.index)
      vector_component_value = 70.0;
    if ((index == Fields::Cel2.index) && (component == 0))
      vector_component_value = 20.0;
    if ((index == Fields::Cel2.index) && (component == 1))
      vector_component_value = 30.0;
    if (index == Fields::Cel3.index)
      vector_component_value = 0.0;
    if (index == Fields::eig1.index)
      vector_component_value = 0.01;
    if (index == Fields::eig2.index)
      scalar_value = 0.0;
  }
  ...
  void compute_rhs(
    ...
    if (solve_block_id == 0) // c
    {
      ...
      ScalarValue func_c = (psi_grad / psi) * mobility_factor * flux;
      ScalarGrad func_c_grad = -mobility_factor * flux;
    ...
    if (solve_block_id == 1) // u
    {
      ...
      ScalarValue psi =
          variable_list.template get_value<Scalar, Current>(Fields::psi.index);
      VectorGrad eigenstrain0 = TensorHelper::extract_rank2_symm(
          variable_list, Subsets::eigenstrain());
      VectorGrad eigenstrain = (c_val - c_ref) * eigenstrain0;
      dealii::Tensor<2, Mechanics::voigt_tensor_size<dim>, ScalarValue>
          stiffness = TensorHelper::extract_stiffness(variable_list,
                                                      Subsets::stiffness());
      VectorGrad stress;
      Mechanics::compute_stress<dim, ScalarValue>(stiffness, psi * eigenstrain,
                                                  stress);
    ...
    if (solve_block_id == 2) // mu
    {
      ...
      VectorGrad ux =
          variable_list.template get_symmetric_gradient<Vector, Current>(
              Fields::u.index);
      ScalarValue psi =
          variable_list.template get_value<Scalar, Current>(Fields::psi.index);
      VectorGrad eigenstrain0 = TensorHelper::extract_rank2_symm(
    ...
    if (solve_block_id == 3) // post-processing
    {
      ...
      variable_list.set_value_term(Fields::particle_concentration.index,
                                   psi * c_val);
      ...
      for (unsigned int k = 0; k < stress[0][0].size(); ++k) {
        ...
        std::array<number, dim> stress_eigvals =
            dealii::eigenvalues(stress_sym);
        for (unsigned int i = 0; i < dim; ++i) {
      ...
      */
      ...
      variable_list.set_value_term(Fields::stress_off_diag.index,
                                   stress_offdiag);
      ...
      ScalarValue elastic_diffpot =
          dealii::scalar_product<2, dim, ScalarValue>(stress, eigenstrain0);
      elastic_diffpot *= site_volume * stress_scale;
      variable_list.set_value_term(Fields::mu_elastic.index,
                                   elastic_diffpot / F);
      variable_list.set_value_term(Fields::mu_chem.index,
                                   ocv_model_.eval_U_ocv(c_val));
    }
  }
  ...
  void compute_lhs(
    ...
    {
      VectorGrad ux =
          variable_list.template get_symmetric_gradient<Vector, LHS>(
              Fields::u.index);
      ScalarValue psi =
          variable_list.template get_value<Scalar, Current>(Fields::psi.index);
      VectorGrad stress;
  ...
  number stress_scale;
  number V_ref;
  number i_target = 0.0; // will be modified in post_solve_block to be the
                         // target current density for the simulation
  number delta_X_Li;
  number C_rate;
  number V_step;
  number V_min;
