...
public:
  ...
  CustomPDE(const UserInputParameters<dim> &_user_inputs, PhaseFieldTools<dim> &_pf_tools)
    ...
    , C11(get_user_inputs().user_constants.get_double("C11"))
    , C22(get_user_inputs().user_constants.get_double("C22"))
    , C66(get_user_inputs().user_constants.get_double("C66"))
    , C12(get_user_inputs().user_constants.get_double("C12"))
    , C16(get_user_inputs().user_constants.get_double("C16"))
    , C26(get_user_inputs().user_constants.get_double("C26"))
    , eig11(get_user_inputs().user_constants.get_double("eig11"))
    , eig22(get_user_inputs().user_constants.get_double("eig22"))
    , eig12(get_user_inputs().user_constants.get_double("eig12"))
    , D11(get_user_inputs().user_constants.get_double("D11"))
    , D22(get_user_inputs().user_constants.get_double("D22"))
    , D12(get_user_inputs().user_constants.get_double("D12"))
  {}
...
private:
  ...
  ScalarValue
  eval_U_ocv(ScalarValue c_val) const
  {
  ...
  ScalarValue
  eval_dU_ocv(ScalarValue c_val) const
  {
    constexpr double ocv_soc_min    = 0.30;
    constexpr double ocv_soc_span   = 0.69;
    constexpr double cheb_map_scale = 2.0;
  ...
  ScalarValue
  get_elastic_mu2D(const VectorGrad  &stress,
                   const ScalarValue &eig11,
                   const ScalarValue &eig22,
                   const ScalarValue &eig12) const
  {
    ScalarValue energy(0.0);
    if (dim == 2)
      {
        energy = eig11 * stress[0][0] + 2.0 * eig12 * stress[0][1] + eig22 * stress[1][1];
      }
    else
      {
  ...
  VectorGrad
  eigenstrain_helper(const ScalarValue &eig11,
                     const ScalarValue &eig22,
                     const ScalarValue &eig12) const
  {
  ...
  get_voigt2D(const ScalarValue &C11,
              const ScalarValue &C22,
              const ScalarValue &C66,
              const ScalarValue &C12,
              const ScalarValue &C16,
              const ScalarValue &C26) const
  {
  ...
  VectorValue
  get_flux2D(const VectorValue &cx,
             const ScalarValue &D11,
             const ScalarValue &D22,
             const ScalarValue &D12) const
  {
    VectorValue flux;
    if (dim == 2)
      {
                   ...
                   [[maybe_unused]] unsigned int                       solver_id) override
  {
    if (solver_id == 2)
      {
        const auto &psi = solve_context.get_solution_indexer().get_solution_vector(3);
        const auto &rxn = solve_context.get_solution_indexer().get_solution_vector(10);
        number      integrated =
    ...
    if (solve_block_id == 0) // c
      {
        ...
        ScalarGrad  mu_grad = variable_list.template get_gradient<Scalar, OldOne>(2);
        ...
        VectorValue flux =
          get_flux2D(mu_grad, D11, D22, D12); // fixed to use params D11, D22, D12
        ...
        variable_list.set_value_term(10, rxn);
      }
    if (solve_block_id == 1) // u
      {
        ...
        /*
        //Replaced with params C11, C22, C66, C12, C16, C26, eig11, eig22, eig12
    ...
    if (solve_block_id == 2) // mu
      {
        ...
        */
        ...
        Mechanics::compute_stress<dim, ScalarValue>(stiffness,
                                                    psi * (eigenstrain),
                                                    stress);
        ScalarValue elastic_diffpot = get_elastic_mu2D(stress, eig11, eig22, eig12);
    ...
    if (solve_block_id == 3) // pp
      {
        ...
        ScalarValue   eig2 = variable_list.template get_value<Scalar,Current>(20);
        */
        VectorGrad eigenstrain0 = eigenstrain_helper(eig11, eig22, eig12);
        VectorGrad eigenstrain  = (c_val - c_ref) * eigenstrain0;
        dealii::Tensor<2, Mechanics::voigt_tensor_size<dim>, ScalarValue> stiffness =
          get_voigt2D(C11, C22, C66, C12, C16, C26);
        VectorGrad stress;
        ...
        variable_list.set_value_term(7, stress[0][0]);
        variable_list.set_value_term(8, stress[1][1]);
        variable_list.set_value_term(9, stress[0][1]);
        variable_list.set_value_term(11, del_phi);
      }
  }
              ...
              [[maybe_unused]] unsigned int solve_block_id) const override
  {
    if (solve_block_id == 1) // mechanics - lhs
      {
        VectorGrad  ux  = variable_list.template get_symmetric_gradient<Vector, LHS>(1);
        ScalarValue psi = variable_list.template get_value<Scalar, Current>(3);
        psi             = std::max(psi, ScalarValue(offset));
        /*
        //Replaced with params C11, C22, C66, C12, C16, C26, eig11, eig22, eig12
  ...
  number V_min;
  number V_max;
  number C11, C22, C66, C12, C16, C26;
  number eig11, eig22, eig12;
  number D11, D22, D12;
};
